module DocxTemplater
  class TemplateProcessor
    ...
    def enter_multiple_values(document, key)
      ...
      begin_row_template = xml.xpath("//w:tr[not(descendant::w:tr) and contains(., '#{begin_row}')]", xml.root.namespaces).first
      end_row_template = xml.xpath("//w:tr[not(descendant::w:tr) and contains(., '#{end_row}')]", xml.root.namespaces).first
